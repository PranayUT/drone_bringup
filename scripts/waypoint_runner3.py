#!/usr/bin/env python3
"""
DJI waypoint mission runner - lat/lon/yaw format, fixed altitude.

Reads lat,lon,yaw waypoints from a text file, uploads a straight-line mission
to the DJI SDK, starts it, and loiters at each waypoint after arrival.
Altitude is fixed at FIXED_ALTITUDE_M for all waypoints.
Orbit radius at each waypoint is locked from live height above takeoff on arrival.

Usage:
    python waypoint_runner3.py <waypoints_file> [--speed <m/s>] [--arrive-radius <m>] [--loiter <s>] [--altitude <m>]

Waypoints file format (one waypoint per line, space-separated, blank/# lines ignored):
    lat lon yaw_rad
    30.1234 -97.5678 3.14159
    Yaw convention: 0 = South, counterclockwise positive (radians).
    Converted internally to DJI North=0, clockwise-positive degrees.
...
FIXED_ALTITUDE_M        = 10.0
DEFAULT_ARRIVE_RADIUS_M = 1.0
DEFAULT_IDLE_SPEED      = 5.0
DEFAULT_LOITER_SECS     = 10.0
MIN_ORBIT_RADIUS        = 2.0
ARRIVE_SAMPLES_REQUIRED = 3
MIN_NAV_SEC_BEFORE_LOITER = 3.0
MISSION_IN_PROGRESS_ACK    = 213
MISSION_OBTAIN_CONTROL_ACK = 209
MISSION_DATA_NOT_ENOUGH_ACK = 234
MAX_HOTPOINT_YAW_RATE_DEG_S = 15.0
...
def yaw_rad_to_dji_deg(yaw_rad):
    """
    Convert yaw from South=0, counterclockwise-positive radians
    to DJI North=0, clockwise-positive degrees in [-180, 180].
    """
...
def parse_waypoints(path, fixed_alt):
    """Return list of (lat, lon, yaw_dji_deg, alt) tuples from a space-separated lat lon yaw_rad file."""
    waypoints = []
    with open(path) as f:
        for lineno, raw in enumerate(f, 1):
            ...
            parts = line.split()
            if len(parts) != 3:
                rospy.logwarn("Line %d: expected 'lat lon yaw_rad' - skipping: %s", lineno, raw.rstrip())
                continue
            try:
                lat, lon, yaw_rad = float(parts[0]), float(parts[1]), float(parts[2])
                yaw_deg = yaw_rad_to_dji_deg(yaw_rad)
                waypoints.append((lat, lon, yaw_deg, fixed_alt))
            except ValueError:
...
def build_waypoint_task(waypoints, idle_speed):
    """Build a dji_sdk/MissionWaypointTask with per-waypoint yaw control."""
...
def continuation_waypoints(waypoints, current_index):
    ...
    cur_alt  = float(_state.height_above_takeoff) if _state.height_valid else remaining[0][3]
    cur_yaw  = remaining[0][2]
    return [(cur_lat, cur_lon, cur_yaw, cur_alt), remaining[0]]
...
def track_and_loiter_waypoints(waypoints, arrive_radius, loiter_secs, orbit_speed, mission_start, action_srv):
    ...
    while not rospy.is_shutdown() and next_wp < total:
        ...
        wp_lat, wp_lon, wp_yaw, wp_alt = waypoints[next_wp]
...
def main():
    parser = argparse.ArgumentParser(description="DJI waypoint mission runner - lat/lon/yaw, fixed altitude")
    parser.add_argument("waypoints_file", help="Path to waypoints text file (lat,lon,yaw per line)")
    ...
    parser.add_argument("--altitude",      type=float, default=FIXED_ALTITUDE_M,
                        help="Fixed altitude in metres for all waypoints (default: %.1f)" % FIXED_ALTITUDE_M)
    args = parser.parse_args(rospy.myargv(argv=sys.argv)[1:])

    rospy.init_node("waypoint_runner3", anonymous=True)

    waypoints = parse_waypoints(args.waypoints_file, args.altitude)
    if len(waypoints) < 2:
        rospy.logfatal("Need at least 2 waypoints; got %d. Aborting.", len(waypoints))
        sys.exit(1)
    rospy.loginfo("Loaded %d waypoints from %s (alt=%.1f m)", len(waypoints), args.waypoints_file, args.altitude)
    for i, (lat, lon, yaw, alt) in enumerate(waypoints):
        rospy.loginfo("  WP%d: lat=%.7f  lon=%.7f  yaw=%.1f deg  alt=%.1f m", i + 1, lat, lon, yaw, alt)
